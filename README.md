...
外部 API に接続しない local-only worker skeleton を含みます。

```bash
python3 -m sv_terminal_worker.cli parse-marker '[SV_APPROVAL action_id=act_1 decision=approved target=proposal:v1 by=hirotea]'
...
python3 -m sv_terminal_worker.cli build-agent-input --context-file .sv-terminal/runs/BAA-1234/act_20260605_001/context.json
python3 -m unittest discover -s tests
```

最初の実装範囲は marker parsing、context packet 生成、PR agent input 生成、idempotency state file 作成です。
実adapter mode では、`--event-source n8n --issue-source linear` を使います。
...
- [Proposal & Delivery Contract](docs/09-proposal-delivery-contract.md)
- [Current Spec Overview](docs/10-current-spec-overview.md)
- [Local Worker PR / Backtest Flow](docs/11-local-worker-pr-backtest-flow.md)
- [Worker Implementation Plan](docs/12-worker-implementation-plan.md)
- [Ports and Adapters Worker Architecture](docs/13-ports-and-adapters-worker-architecture.md)
- [Linear / n8n Adapter Contract](docs/14-linear-n8n-adapter-contract.md)
- [Linear Trigger → n8n → Local Worker 検証手順](docs/15-linear-trigger-n8n-verification.md)
- [Desktop App / Chrome Extension SV Actions](docs/16-desktopapp-chrome-extension-actions.md)
